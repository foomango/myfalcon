...
enum {
    ...
    /* jbl: These are now in milliseconds. Also, they don't matter */
...
class cohort : public virtual refcount {
 ...
 private:
	...
	// Uncontrollably lame
        ...
        warn << "Got a failure " << domain << ":" << host << "\n";
	...
	for (viewstamp_t vs = vs_next(executed_); vs <= committed_; vs = vs_next(vs)) {
		...
		if (view_.primary.mid == self_.mid) {
		    warn << "This cohort is primary in " << view_.vid << "\n";
		}
		for (int i = 0; i < view_.backups.size(); i++) {
			mid_t m = view_.backups[i].mid;
			if (m != self_.mid && mid2fdfd_[m] == 0) {
				warn << "There goes a falcon:" << view_.backups[i].name << " " << view_.backups[i].hypervisor << "\n";
				warn << m << " is who it's after\n";
		...
		if (m != self_.mid && mid2fdfd_[m] == 0) {
			warn << "There goes a falcon:" << view_.primary.name << " " << view_.primary.hypervisor << "\n";
			warn << m << " is who it's after\n";
			mid2fdfd_[m] = -1; // Let's us know we're already getting the connection
